...
class AppointmentSlots extends Model
{
	...
	public static function getSloat2($id='', $date, $type=1)
	{
		...
		for($i=0; $i<=3; $i++){
			...
			foreach ($unique_sloat as $key => $value) {  
				...
				if($checkSloat==null){ 
					...
					if($type==1) {
						echo '<li><a href="'. route('doctor.details', [$checkSloat->doctor_id,$checkSloat->id,date("Ymd",strtotime($checkSloat->slot_date))]).'">'.date('d-m-Y h:i a', strtotime($checkSloat->slot_time)).'</a></li>';
					} else {
						echo '<li><a href="javascript:void(0)" onclick="setsloat2('.$checkSloat->id.')" >'.date('h:i a', strtotime($checkSloat->slot_time)).'</a></li>';
					}
			...
			if($slot_count>4){
				...
				if($check_more_sloat>0){
					echo '<li><a href="javascript:void(0)" onclick="more_desktop('.$id.','.date("Ymd",strtotime($date)).')">More</a></li>';
				}else{
					echo '<li><a href="javascript:void(0)" class="empty">--</a></li>';
				} 
			}
