...
class LocalityController extends Controller{
    ...
    public function import(Request $request,$countryid,$stateid,$cityid){
        ...
        if ($request->all()){
            ...
            $data['cityid'] = $cityid;
            return view('Locality.import', $data);
        }
    }
   ...
   public function export($countryid,$stateid,$cityId)
   {
    ...
    {
        $data['data'] = Location::where([['country_id',$countryid],['state_id',$stateid],['city_id',$cityId]])->select('id','name','pincode','active')->get();
        $data['active'] = 'locations';
        return view('Locality.index', $data);
    }

    public function edit(int $country_id = 0)
    {
        try {
            $data['data'] = Location::find($country_id);
            if($data['data'] == null) { // If details not found then return
                return redirect()->back()->with('error', 'Details not found');
            }
            $data['active'] = 'locations';
            return view('Locality.edit', $data);
        } catch(\Exception $e) {
